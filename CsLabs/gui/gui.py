...
def check():
    ...
    # file.close()
    frame2 = Frame(main, bd=10, bg='#AF3FA6', highlightthickness=3)
    frame2.config(highlightbackground="white")
    frame2.place(relx=0.5, rely=0.1, width=800, relwidth=0.4, relheight=0.8, anchor='n')

    text2 = Text(frame2, bg="#AF3FA6", width=50, height=27.5, highlightthickness=3)
    text2.place(relx=0.07, rely=0.03, relwidth=0.4, relheight=0.9)
    text2.insert(END, '\n\n'.join(success))

    listbox_fail = Listbox(frame2, bg="#AF3FA6", font=myFont, fg="white", listvariable=valori2, selectmode=MULTIPLE,
    ...
    exit_btn = Button(frame2, text='Close', command=exit, bg="#AF3FA6", fg="white", font=myFont, padx='10px',
                      pady='3px')
    ...
    changeBtn = Button(frame2, text='Change', command=changeFailures, bg="#AF3FA6", fg="white", font=myFont,
                       padx='10px',
                       pady='3px')
    changeBtn.place(relx=0.66, rely=0.95)

    backupBtn = Button(frame2, text='Restore', command=restore, bg="#AF3FA6", fg="white", font=myFont,
                       padx='10px',
                       pady='3px')
...
lstbox = Listbox(frame, bg="#AF3FA6", font=myFont, fg="white", listvariable=valori, selectmode=MULTIPLE, width=130,
...
def LoginToFB():
    ...
    def PassInfo():
        ...
        emailElem.send_keys(
            E_mail_value)  # This finds element of email field and then it takes our new strings and passes them on.
        sleep(0.5)
...
text = Text(frame, bg="#AF3FA6", fg="white", font=myFont, width=50, height=26, highlightthickness=3)
text.config(highlightbackground="white")
text.grid(row=0, column=3, columnspan=3, padx=30)
import_button = Button(frame, bg="#AF3FA6", fg="white", font=myFont, text="Import", width=7, height=1,
                       command=import_audit).place(relx=0.01, rely=0.999)  # y was 435
openButton = Button(frame, bg="#AF3FA6", fg="white", font=myFont, text="Save", width=7, height=1,
                    command=save_config).place(relx=0.06, rely=0.999)
selectAllButton = Button(frame, bg="#AF3FA6", fg="white", font=myFont, text="Select All", width=7, height=1,
                         command=select_all).place(relx=0.11, rely=0.999)
deselectAllButton = Button(frame, bg="#AF3FA6", fg="white", font=myFont, text="Deselect All", width=10, height=1,
...
search_button = Button(frame, bg="#AF3FA6", fg="white", font=myFont, text="Search", width=7, height=1,
                       command=search).place(relx=0.49, rely=0.999)
check_button = Button(frame, bg="#AF3FA6", fg="white", font=myFont, text="Check", width=7, height=1,
